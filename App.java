...
// Method to generate library statistics report
    ...
    System.out.println("Total Borrowed Books: " + transactionCount);
    System.out.println("Total Available Books: " + (bookCount - transactionCount));
    ...
    static double totalFines() {
        ...
        for (int i = 0; i < transactionCount; i++) {
            totalFine += transactionFineAmounts[i]; // Example fine calculation
        }
        return totalFine;
    }

 // Method to member Activity Report
    static void generateMemberActivityReport() {
       
        System.out.println("\n========== MEMBER ACTIVITY REPORT ==========");
        System.out.println();

        if (transactionCount == 0) {
            System.out.println("No Activity to Report.");
            return;
        }

        // Display transaction details
        for (int i = 0; i < transactionCount; i++) {
          if (transactionReturnDates[i].isEmpty()) {
            
            System.out.println("Member ID: " + transactionMemberIds[i]);
            System.out.println("Book ID: " + transactionBookIds[i]);
         
            System.out.println("Borrow Date: " + transactionBorrowDates[i]);
            System.out.println("Due Date: " + transactionDueDates[i]);
        ...
        for (int i = 0; i < transactionCount; i++) {
            System.out.printf("%-15s %-15s %-15s %-15s %-15s %-15s %-15s%n",
                    "Transaction ID", "Member ID", "Book ID", "Borrow Date", "Due Date", "Return Date", "Fine Amount");
            System.out.printf("%-15s %-15s %-15s %-15s %-15s %-15s %-15s%n",
                    transactionIds[i], transactionMemberIds[i], transactionBookIds[i],
                    transactionBorrowDates[i], transactionDueDates[i], transactionReturnDates[i],
                    transactionFineAmounts[i]);
            System.out.println("-------------------------------");
        }

        //totalFines(); // Call to calculate total fines


    }


// Method to generate popular books report
    static void generatePopularBooksReport() {
        System.out.println("\n========== POPULAR BOOKS REPORT ==========");
        System.out.println();

        if (transactionCount == 0) {
            System.out.println("No Transactions have been done.");
            return;
        }

        // Count book popularity  // --------------------------  Generated by copilot ----------------------------
        Map<String, Integer> bookPopularity = new HashMap<>();
        for (int i = 0; i < transactionCount; i++) {
            String bookId = transactionBookIds[i];
            bookPopularity.put(bookId, bookPopularity.getOrDefault(bookId, 0) + 1);
        }

        // Sort books by popularity
        List<Map.Entry<String, Integer>> sortedBooks = new ArrayList<>(bookPopularity.entrySet());
        sortedBooks.sort((a, b) -> b.getValue().compareTo(a.getValue()));

        // Display top 5 popular books
        System.out.printf("%-15s %-15s%n", "Book ID", "Borrow Count");
        ...
        //Display least popular book  // ------------------------ Generated by copilot -------------------------------
        System.out.println("\nLeast Popular Book:");
        System.out.printf("%-15s %-15s%n", "Book ID", "Borrow Count");
        for (int i = sortedBooks.size() - 1; i >= Math.max(0, sortedBooks.size() - 5); i--) {
            Map.Entry<String, Integer> entry = sortedBooks.get(i);
            System.out.printf("%-15s %-15d%n", entry.getKey(), entry.getValue());
        }
...
//Method to generate overdue books report
    static void generateOverdueBooksReport() {
        ...
        for (int i = 0; i < transactionCount; i++) {
            if (transactionReturnDates[i].isEmpty()) {
                continue; // Skip transactions that are not returned yet
            }
            LocalDate dueDate = LocalDate.parse(transactionDueDates[i], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalDate returnDate = LocalDate.parse(transactionReturnDates[i], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            ...
            if (returnDate.isAfter(dueDate)) {
                ...
                System.out.printf("Transaction ID: %d,  Book ID: %s, Book Name: %s",
                        transactionIds[i],
                        transactionBookIds[i],
                        "<Book Name>");
            ...
            System.out.println();
            double accumulatedFine = 0;
            accumulatedFine += transactionFineAmounts[i];
            System.out.println("Accumulated fines: " + accumulatedFine);

            System.out.println();
            System.out.println("----------------- Member Contact Details -----------------");

            for (int j = 0; j < memberCount; j++) {
                if (memberIds[j] == transactionMemberIds[i]) {
                    System.out.printf("Member ID: %d, Name: %s, Phone: %s%n",
                            memberIds[j], memberNames[j], memberPhoneNumbers[j]);
                    return;
                }
            }
